...
public abstract class BaseOperatorTest extends BaseExpressionTest {
	...
	public void getOperatorSymbol_returns_expected_symbol() {
		assertEquals(getSymbol(), ((PairNode) build()).getOperatorSymbol());
	}

	@Test
	public void setting_left_to_Equal_node_returns_null() {
	...
	@Test
	public void setting_right_to_Equal_node_returns_null() {
		assumeFalse("This test is applicable to operators except Equals", clazz == Equals.class);
		assertThat(nodeBuilder.left(getLeft()).right(new Equals(c1, h1)).build(), is(nullValue()));
	}

	private Expression build() {
		Expression pairNode = nodeBuilder.build(getLeft(), getRight());
		assertThat(pairNode, is(not(nullValue())));
		return pairNode;
	}
}
