...
public class MoveTest extends BaseTest {
	...
	public void verify_properties_of_capture_move() {
		...
		assertThat(move.toString(), is(equalTo("CAPTURE (1C+7C) 8C")));
	}
	...
	public void verify_properties_of_build_move() {
		...
		assertThat(move.isPileGenerated(), is(true));
		assertThat(move.toString(), is(equalTo("BUILD (8C-7C) 8C")));
	}
	...
	public void create_returns_move_for_valid_arguments() {
		...
		Card handCard = c1;
		Move move = Move.create(type, expression, handCard, Collections.<Pile> emptySet(), Collections.singleton(c1), player1);
		assertThat(move, is(equalTo(new Move(type, expression, handCard))));
	}

	@Test
	public void create_throws_for_null_type() {
		assertExceptionForCreate(null, c1, null, Collections.<Pile> emptySet(), Collections.singleton(c1), player1, "type");
	}

	@Test
	public void create_throws_for_null_hand_card() {
		assertExceptionForCreate(DISCARD, null, null, Collections.<Pile> emptySet(), Collections.singleton(c1), player1, "handCard");
	}

	@Test
	public void create_throws_for_null_table() {
		assertExceptionForCreate(DISCARD, c1, null, null, Collections.singleton(c1), player1, "table");
	}

	@Test
	public void create_throws_for_null_hand() {
		assertExceptionForCreate(DISCARD, c1, null, Collections.<Pile> emptySet(), null, player1, "hand");
	}
	...
	public void create_returns_null_when_expression_is_not_valid() {
		Move move = Move.create(DISCARD, c2, c1, Collections.<Pile> emptySet(), Collections.singleton(c1), player1);
		assertThat(move, is(nullValue()));
	}

	@Test
	public void create_returns_null_when_move_is_not_valid_for_specified_table_hand_and_player() {
		Move move = Move.create(DISCARD, null, c1, Collections.<Pile> emptySet(), Collections.singleton(c2), player1);
		assertThat(move, is(nullValue()));
	}
